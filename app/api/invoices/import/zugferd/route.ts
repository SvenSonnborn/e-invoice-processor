  ...
  try {
    ...
    if (files.length > MAX_BATCH_FILES)
      return NextResponse.json(
        {
          success: false,
          error: `Too many files. Maximum ${MAX_BATCH_FILES} per request.`,
        },
        { status: 400 }
      );
    for (const file of files) {
      if (file.size > MAX_FILE_SIZE)
        return NextResponse.json(
          {
            success: false,
            error: `File "${file.name}" too large. Maximum ${MAX_FILE_SIZE / 1024 / 1024} MB.`,
          },
          { status: 400 }
        );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'File upload failed', details: String(error) },
      { status: 400 }
    );
  }
}
  ...
  try {
    ...
    if (Array.isArray(invoices)) {
      if (invoices.length === 0)
        return NextResponse.json(
          { success: false, error: 'Empty "invoices" array' },
          { status: 400 }
        );
      if (invoices.length > MAX_BATCH_FILES)
        return NextResponse.json(
          {
            success: false,
            error: `Too many items. Maximum ${MAX_BATCH_FILES} per request.`,
          },
          { status: 400 }
        );
      ...
      for (let index = 0; index < invoices.length; index++) {
        ...
        if (!item || typeof item.xml !== 'string')
          throw new Error(`invoices[${index}]: missing or invalid "xml" field`);
        const buffer =
          item.format === 'pdf'
    ...
    if (!xml || typeof xml !== 'string')
      return NextResponse.json(
        { success: false, error: 'Missing or invalid "xml" field' },
        { status: 400 }
      );
    let result;
    if (format === 'pdf')
    ...
    return NextResponse.json(
      { success: false, error: 'Invalid JSON body', details: String(error) },
      { status: 400 }
    );
  }
}
