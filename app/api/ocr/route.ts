    ...
    const response: OcrApiResponse = {
      ...
      data: {
        ...
        invoice: {
          ...
          totalAmount: invoiceData.totals?.grossAmount ? Number(invoiceData.totals.grossAmount) : undefined,
          currency: invoiceData.totals?.currency,
          taxAmount: invoiceData.totals?.taxAmount ? Number(invoiceData.totals.taxAmount) : undefined,
          lineItems: (invoiceData as unknown as { lineItems?: Array<{ description: string; quantity: number; unitPrice: number; total: number }> }).lineItems?.map(item => ({
