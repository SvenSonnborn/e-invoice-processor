...
import { mapToInvoiceModel, mapToExtendedInvoiceData } from "@/src/lib/zugferd/mapper";
...
describe("Invoice Mapper", () => {
  const sampleZugferd: ZUGFeRDInvoice = {
    ...
    currency: "EUR",
    paymentTerms: { dueDate: "2024-02-15", description: "Payment within 30 days" },
  ...
  describe("mapToInvoiceModel", () => {
    ...
    it("should map XRECHNUNG format when profile contains xrechnung", () => {
      const xr = { ...sampleZugferd, metadata: { ...sampleZugferd.metadata, profile: "urn:xoev-de:kosit:standard:xrechnung_2.3" } };
  ...
  describe("mapToExtendedInvoiceData", () => {
    ...
    it("should handle data without payment info", () => {
      const noPayment = { ...sampleZugferd, paymentTerms: undefined, payeeIban: undefined, payeeBic: undefined };
