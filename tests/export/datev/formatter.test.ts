...
import type {
  ...
  DatevInvoice,
  DatevExportConfig,
} from '../../../src/lib/export/datev/types';
import {
  validateDatevEntry,
...
} from '../../../src/lib/export/datev/constants';
import {
  ...
  describe('Mapper', () => {
    describe('mapTaxRateToSteuerschluessel', () => {
      it('should map 19% to standard tax key', () => {
        expect(
          mapTaxRateToSteuerschluessel(19, DEFAULT_INVOICE_MAPPING, true)
        ).toBe(DATEV_STEUERSCHLUESSEL.VORSTEUER_19);
        // With DEFAULT_INVOICE_MAPPING, steuerschluesselStandard ("9") always takes precedence
        expect(
          mapTaxRateToSteuerschluessel(19, DEFAULT_INVOICE_MAPPING, false)
        ).toBe(DEFAULT_INVOICE_MAPPING.steuerschluesselStandard);
      });

      it('should map 7% to reduced tax key', () => {
        expect(
          mapTaxRateToSteuerschluessel(7, DEFAULT_INVOICE_MAPPING, true)
        ).toBe(DATEV_STEUERSCHLUESSEL.VORSTEUER_7);
      });

      it('should map 0% to zero tax key', () => {
        expect(
          mapTaxRateToSteuerschluessel(0, DEFAULT_INVOICE_MAPPING, true)
        ).toBe(DATEV_STEUERSCHLUESSEL.VORSTEUER_0);
      });
    });
    ...
    describe('mapInvoiceToDatevEntries', () => {
      ...
      it('should create entries for incoming invoice', () => {
        ...
        expect(firstEntry.konto).toBe(
          DEFAULT_INVOICE_MAPPING.kontoEingangsrechnung
        );
      ...
      it('should create entries for outgoing invoice', () => {
        ...
        const firstEntry = entries[0];
        expect(firstEntry.konto).toBe(
          DEFAULT_INVOICE_MAPPING.kontoAusgangsrechnung
        );
      });

      it('should include tax entry when tax amount > 0', () => {
        const entries = mapInvoiceToDatevEntries(baseInvoice);
        const taxEntry = entries.find(
          (e) => e.steuerschluessel === DATEV_STEUERSCHLUESSEL.VORSTEUER_19
        );
