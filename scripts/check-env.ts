...
function checkEnv() {
  ...
  for (const varName of todoEnvVars) {
    ...
    if (value) {
      ...
      const masked = maskSensitiveValue(varName, value);
      console.log(`  ⚠️  ${varName} is set: ${masked} (not yet used by application)`);
      hasWarnings = true;
    }
  }
  if (setTodoVars.length === 0) {
    console.log("  ℹ️  No TODO variables are set (this is fine, they're not implemented yet)");
  }
  ...
  if (hasErrors) {
    ...
    console.log("⚠️  Some TODO variables are set (they won't be used until implemented)");
    process.exit(0);
  } else {
    console.log("✅ All required environment variables are set!");
    process.exit(0);
