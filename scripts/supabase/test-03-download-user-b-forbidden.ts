...
async function main() {
  ...
  if (!error) {
    console.error("❌ Download should have failed (User B accessing Org A file)");
    if (data) console.error("   Received", (data as Blob).size, "bytes");
    process.exit(1);
  }

  const msg = (error?.message ?? (error as { error?: string })?.error ?? "").toLowerCase();
  const code = (error as { statusCode?: number })?.statusCode ?? (error as { code?: string })?.code;
  const codeNum = typeof code === "string" ? parseInt(code, 10) : code;
  const is403 = codeNum === 403 || /403|forbidden|unauthorized/i.test(msg);
  const isRls = /row level security|rls|policy|denied|violation|objects.*not found/i.test(msg);
