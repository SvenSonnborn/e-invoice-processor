...
export function GoBDBadge({
  ...
  return (
    <div className={cn('relative inline-block', className)}>
      <button
        onClick={() => total && setExpanded(!expanded)}
        className={cn(
        ...
        {total > 0 && (
          <span
            className={cn(
              ...
              status === 'non-compliant' ? 'bg-red-200' : 'bg-yellow-200'
            )}
          >
            {total}
          </span>
      ...
      {expanded && total > 0 && (
        <div
          className={cn(
            'absolute left-0 top-full z-50 mt-1 w-80 rounded-lg border bg-white p-3 shadow-lg',
            c.border
          )}
        >
          {violations.length > 0 && (
            <div className="mb-2">
              <h4 className="mb-1 font-semibold text-red-700 text-sm">
                Fehler ({violations.length})
              </h4>
          ...
          {warnings.length > 0 && (
            <div>
              <h4 className="mb-1 font-semibold text-yellow-700 text-sm">
...
export function GoBDStatusIcon({
  ...
  return (
    <Icon
      className={cn('h-5 w-5', c.iconColor, className)}
      aria-hidden="true"
    />
  );
}
