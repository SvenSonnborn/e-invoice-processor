...
function lineItemToDatevRow(
  ...
  const account =
    item.accountNumber ??
    (isOutgoingInvoice ? defaultRevenueAccount : defaultExpenseAccount);
  ...
  const { amount: formattedAmount, sign } = formatAmountWithSign(
    amount,
    isOutgoingInvoice
  );
  ...
  // Steuerschlüssel
  const taxKey =
    item.taxKey ?? getTaxKey(Number(item.taxRate), !isOutgoingInvoice);

  // Buchungstext
  const bookingText =
    item.bookingText ??
    `${invoice.number ?? `RG-${lineIndex}`} ${item.description ?? ''}`.substring(
      0,
      60
    );
...
export function invoicesToDatevCsv(
  ...
  const dataLines = items.map((item, index) =>
    lineItemToDatevRow(item, options, index)
  );
...
export function generateDatevFilename(
  ...
  const { consultantNumber = '0000000', clientNumber = '00000' } = options;
