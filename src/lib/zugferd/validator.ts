...
const SCHEMA_URLS: Record<string, string> = {
  ...
  'zugferd-2.3-en16931':
    'https://www.zugferd.org/schemas/ZF23/FACTUR-X_EN16931.xsd',
  'xrechnung-cii':
    'https://www.xrechnung-bund.de/de/leitweg/validation/XRechnung-CII.xsd',
  'xrechnung-ubl':
...
export async function validateXML(
  ...
  if (!wellFormedResult.valid)
    return { valid: false, errors: wellFormedResult.errors, warnings };
  ...
  if (libxmljs) {
    try {
      const schemaResult = await validateAgainstSchema(
        xmlContent,
        flavor,
        version,
        profile
      ...
      warnings.push(
        `Schema validation skipped: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  } else {
...
function checkWellFormed(xmlContent: string): {
  valid: boolean;
  errors: string[];
} {
  try {
    ...
    return {
      valid: false,
      errors: [
        `XML is not well-formed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      ],
    };
  }
}
...
function validateRequiredFields(
  ...
  try {
    ...
    else if (flavor === 'XRechnung')
      validateUBLFields(parsed, errors, warnings);
  } catch (error) {
    errors.push(
      `Field validation error: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
...
function validateCIIFields(
  ...
  const supplyChainTrade = asRecord(
    cii['rsm:SupplyChainTradeTransaction'] || cii.SupplyChainTradeTransaction
  );

  if (!getTextContent(header?.['ram:ID'] || header?.ID))
    errors.push('Missing required field: Document ID');
  if (!getTextContent(header?.['ram:TypeCode'] || header?.TypeCode))
    errors.push('Missing required field: Document Type Code');
  ...
  const seller = asRecord(
    agreement?.['ram:SellerTradeParty'] || agreement?.SellerTradeParty
  );
  const buyer = asRecord(
    agreement?.['ram:BuyerTradeParty'] || agreement?.BuyerTradeParty
  );

  if (!seller) errors.push('Missing required field: Seller Trade Party');
  else {
    const sellerLegalOrg = asRecord(
      seller['ram:SpecifiedLegalOrganization'] ||
    ...
    if (
      !getTextContent(
        seller['ram:Name'] ||
          seller.Name ||
          sellerLegalOrg?.['ram:TradingBusinessName'] ||
          sellerLegalOrg?.TradingBusinessName
      )
    ) {
  ...
  if (!buyer) errors.push('Missing required field: Buyer Trade Party');
    ...
    const buyerLegalOrg = asRecord(
      buyer['ram:SpecifiedLegalOrganization'] ||
        buyer.SpecifiedLegalOrganization
    );
    if (
      !getTextContent(
        buyer['ram:Name'] ||
          buyer.Name ||
          buyerLegalOrg?.['ram:TradingBusinessName'] ||
          buyerLegalOrg?.TradingBusinessName
      )
    ) {
  ...
  if (
    ...
    errors.push('Missing required field: Invoice Currency Code');
  ...
  if (!summation) errors.push('Missing required field: Monetary Summation');
    ...
    errors.push('Missing required field: Grand Total Amount');
}

function validateUBLFields(
  parsed: Record<string, unknown>,
  errors: string[],
  _warnings: string[]
): void {
  ...
  if (!invoice) {
    errors.push('Missing root element: Invoice');
    return;
  }

  if (!getTextContent(invoice['cbc:ID'] || invoice.ID))
    errors.push('Missing required field: Invoice ID');
  if (!getTextContent(invoice['cbc:IssueDate'] || invoice.IssueDate))
    errors.push('Missing required field: Issue Date');
  if (
    !getTextContent(
      invoice['cbc:DocumentCurrencyCode'] || invoice.DocumentCurrencyCode
    )
  )
    errors.push('Missing required field: Document Currency Code');

  const supplier = asRecord(
    invoice['cac:AccountingSupplierParty'] || invoice.AccountingSupplierParty
  );
  if (!supplier)
    errors.push('Missing required field: Accounting Supplier Party');
  else {
    const party = asRecord(supplier['cac:Party'] || supplier.Party);
    const partyLegalEntity = asRecord(
  ...
  if (!customer)
    errors.push('Missing required field: Accounting Customer Party');
  else {
  ...
  if (!legalTotal) errors.push('Missing required field: Legal Monetary Total');
    ...
    !getTextContent(legalTotal['cbc:PayableAmount'] || legalTotal.PayableAmount)
  )
    errors.push('Missing required field: Payable Amount');
}
...
async function validateAgainstSchema(
  ...
  try {
    ...
    if (!schemaContent) {
      warnings.push(
        `Schema not cached: ${schemaUrl}. Schema validation skipped.`
      );
      return { valid: true, errors, warnings };
    }
    ...
    if (!isValid) {
      (xmlDoc.validationErrors || []).forEach((err) =>
        errors.push(`Schema validation: ${err.message} (line ${err.line})`)
      );
    }
  } catch (error) {
    errors.push(
...
export function getValidationInfo(xmlContent: string): {
  flavor: InvoiceFlavor;
  version?: string;
  profile?: string;
} {
