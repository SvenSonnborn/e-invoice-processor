  ...
  try {
    if (!isPDF(pdfBuffer)) {
      return {
        ...
        detection: { flavor: 'Unknown' },
        errors: ['Invalid PDF file: File does not have PDF header'],
        warnings,
      };
    }
    ...
    try {
      ...
      const errorMsg =
        error instanceof ZUGFeRDParserError ? error.message : 'Unknown error';
      return {
        success: false,
        validation: {
          valid: false,
          errors: [`XML extraction failed: ${errorMsg}`],
          warnings: [],
        },
        detection: { flavor: 'ZUGFeRD' },
        errors: [`Failed to extract XML from PDF: ${errorMsg}`],
        warnings,
      };
    }
    ...
    return {
      success: false,
      validation: { valid: false, errors: [errorMsg], warnings: [] },
      detection: { flavor: 'Unknown' },
...
export async function parseInvoiceFromXML(
  xmlContent: string
): Promise<InvoiceParseResult> {
  const errors: string[] = [];
  const warnings: string[] = [];
  ...
  try {
    ...
    try {
      ...
      const errorMsg =
        error instanceof MapperError ? error.message : 'Mapping failed';
    ...
    return {
      success: errors.length === 0 && !!invoice,
      invoice,
      extendedData,
      rawData: parseResult.invoice,
      validation,
      detection,
      errors,
      warnings,
    ...
    return {
      ...
      validation: { valid: false, errors: [errorMsg], warnings: [] },
      detection: { flavor: 'Unknown' },
      errors: [`XML parsing failed: ${errorMsg}`],
      warnings,
    };
  }
}
...
export async function parseInvoice(
  ...
  if (isPdfFile || mimeType === 'application/pdf')
    return parseInvoiceFromPDF(buffer);

  let xmlContent: string;
  if (Buffer.isBuffer(buffer)) xmlContent = buffer.toString('utf-8');
  else if (buffer instanceof ArrayBuffer)
    xmlContent = new TextDecoder('utf-8').decode(buffer);
    ...
    return {
      ...
      validation: {
        valid: false,
        errors: ['Invalid buffer type'],
        warnings: [],
      },
      detection: { flavor: 'Unknown' },
      errors: ['Invalid buffer type provided'],
      warnings: [],
    };
...
export async function isValidEInvoice(
  buffer: Buffer | ArrayBuffer | Uint8Array
): Promise<{ valid: boolean; flavor?: InvoiceFlavor; error?: string }> {
  try {
    const result = await parseInvoice(buffer);
    return {
      valid: result.success,
      flavor: result.detection.flavor,
