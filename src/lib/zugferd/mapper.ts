...
export function mapToInvoiceModel(zugferdInvoice: ZUGFeRDInvoice, _sourceFileName?: string): Invoice {
  try {
    ...
    return {
      ...
      totals: mapTotals(zugferdInvoice.monetarySummation, zugferdInvoice.currency),
    };
  } catch (error) {
    throw new MapperError('Failed to map ZUGFeRD invoice to Invoice model', error instanceof Error ? error : undefined);
  }
}
...
function determineInvoiceFormat(zugferdInvoice: ZUGFeRDInvoice): InvoiceFormat {
  ...
  if (profile.includes('xrechnung') || profile.includes('x-rechnung')) return 'XRECHNUNG';
  if (zugferdInvoice.metadata?.flavor) return 'ZUGFERD';
  return 'UNKNOWN';
}

function mapParty(zugferdParty: ZUGFeRDParty | undefined): InvoiceParty | undefined {
  if (!zugferdParty) return undefined;
  return { name: zugferdParty.name || zugferdParty.id || 'Unknown' };
}

function mapTotals(summation: ZUGFeRDMonetarySummation | undefined, currency: string | undefined): InvoiceTotals | undefined {
...
export interface ExtendedInvoiceData {
  ...
  paymentDetails?: { terms?: string; dueDate?: string; meansCode?: string; iban?: string; bic?: string; accountName?: string; mandateId?: string; };
...
export function mapToExtendedInvoiceData(zugferdInvoice: ZUGFeRDInvoice): ExtendedInvoiceData {
  return {
    ...
    supplierDetails: zugferdInvoice.seller ? {
      address: { line1: zugferdInvoice.seller.addressLine1, line2: zugferdInvoice.seller.addressLine2, line3: zugferdInvoice.seller.addressLine3, postcode: zugferdInvoice.seller.postcode, city: zugferdInvoice.seller.city, countryCode: zugferdInvoice.seller.countryCode },
      contact: { name: zugferdInvoice.seller.contactName, phone: zugferdInvoice.seller.contactPhone, email: zugferdInvoice.seller.contactEmail },
      vatId: zugferdInvoice.seller.vatId,
    } : undefined,
    customerDetails: zugferdInvoice.buyer ? {
      address: { line1: zugferdInvoice.buyer.addressLine1, line2: zugferdInvoice.buyer.addressLine2, line3: zugferdInvoice.buyer.addressLine3, postcode: zugferdInvoice.buyer.postcode, city: zugferdInvoice.buyer.city, countryCode: zugferdInvoice.buyer.countryCode },
      vatId: zugferdInvoice.buyer.vatId,
    } : undefined,
    paymentDetails: { terms: zugferdInvoice.paymentTerms?.description, dueDate: zugferdInvoice.paymentTerms?.dueDate || zugferdInvoice.dueDate, meansCode: zugferdInvoice.paymentMeansCode, iban: zugferdInvoice.payeeIban, bic: zugferdInvoice.payeeBic, accountName: zugferdInvoice.payeeAccountName, mandateId: zugferdInvoice.paymentTerms?.directDebitMandateId },
