...
export function generateHeader(
  _config: Partial<DatevExportConfig> = {}
): string {
  const now = new Date();
...
export function generateExtendedHeader(config: DatevExportConfig): string[] {
  ...
  if (errors.length > 0) {
    throw new Error(
      `Invalid export configuration: ${errors.map((e) => e.message).join(', ')}`
    );
  }
...
export function generateRow(entry: DatevEntry): string {
  ...
  if (errors.length > 0) {
    throw new Error(
      `Invalid DATEV entry: ${errors.map((e) => `${e.field}: ${e.message}`).join('; ')}`
    );
  }
 ...
 */
export function generateExtendedCSV(
  entries: DatevEntry[],
