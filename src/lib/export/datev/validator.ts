...
export function validateDatevEntry(entry: DatevEntry): DatevValidationError[] {
  ...
  if (
    !entry.gegenkonto ||
    entry.gegenkonto.length > DATEV_CONSTRAINTS.MAX_GEGENKONTO_LENGTH
  ) {
    errors.push({
      field: 'gegenkonto',
  ...
  if (entry.umsatzSoll === 0 && entry.umsatzHaben === 0) {
    errors.push({
      field: 'umsatz',
      message:
        'Entweder Umsatz Soll oder Umsatz Haben muss einen Wert ungleich 0 haben',
    });
  }

  if (
    entry.buchungstext &&
    entry.buchungstext.length > DATEV_CONSTRAINTS.MAX_BUCHUNGSTEXT_LENGTH
  ) {
    errors.push({
      field: 'buchungstext',
